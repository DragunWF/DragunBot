...
class Stats(commands.Cog):
    ...
    async def execute(self, interaction: discord.Interaction):
        ...
        # embed = discord.Embed(
        #     title=f"{username}'s Stats", color=Utils.get_color("royal blue"))
        # embed.add_field(name="Commands Executed",
        #                 value=f"`{data[Keys.COMMANDS_EXECUTED.value]}`", inline=False)
        # embed.add_field(name="Trivia Points",
        #                 value=f"`{data[Keys.TRIVIA_POINTS.value]}`", inline=False)
        # embed.add_field(name="Times Counted",
        #                 value=f"`{data[Keys.TIMES_COUNTED.value]}`", inline=False)
